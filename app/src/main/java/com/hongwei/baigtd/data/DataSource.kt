package com.hongwei.baigtd.data

import android.content.Context
import android.content.SharedPreferences
import com.google.gson.Gson
import com.hongwei.baigtd.ApiConfig.DataStoreKey.KEY_API_KEY
import com.hongwei.baigtd.ApiConfig.DataStoreKey.KEY_SHARED_SECRET
import com.hongwei.baigtd.ApiConfig.DataStoreKey.KEY_TOKEN
...
class DataSource @Inject constructor(@AppContext context: Context) {
    ...
    fun retriveTokenLocal(): Token? {
        val gson = Gson()
        val json = com.hongwei.baigtd.data.model.Token.json
        val token = gson.fromJson<Any>(json, Token::class.java)
        if (token != null) {
            return token as Token
        }
